...
CREATE TABLE IF NOT EXISTS cursors (
    ...
    event_seq VARCHAR(255) NOT NULL CHECK (event_seq IS NOT NULL AND event_seq <> ''), -- 事件序列号
    tx_digest VARCHAR(255) NOT NULL CHECK (tx_digest IS NOT NULL AND tx_digest <> ''), -- 交易摘要
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
